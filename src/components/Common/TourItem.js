  ...
  return (
    ...
    >
      ...
      <div className='absolute bottom-0 info md:pl-[1.5vw] md:pr-[0.94vw] px-[3.2vw] md:pb-[1.19vw] pb-[3.2vw]     w-full'>
        {!loading ? <div className='flex items-center gap-x-[0.25vw]'>
          <Image
            src={imgLocation}
            width={100}
            height={100}
        ...
        <Link
          ...
          className={`text-white block line-clamp-2 ${!loading ? 'title-tour' : ''} md:text-[1.125vw] text-[2.93vw] font-bold tracking-tight leading-[1.2] mt-[0.25vw]`}
        >
        ...
        <div className='flex items-center justify-between md:mt-[0.81vw] mt-[2.13vw]'>
          {!loading ? <span className='text-primaryColor text-[2.67vw] md:text-[1vw]'>
            ${tourData?.price?.highestPrice} - ${tourData?.price?.lowestPrice}
          </span> : <Skeleton variant="rectangular" width={'80%'} height={50} />}
          {!loading ? <div className='text-[#434447] md:gap-x-[0.2vw] gap-x-[0.8vw] flex items-center md:text-[0.75vw] text-[2.67vw] bg-white md:py-[0.19vw] md:px-[0.5vw] px-[1.28vw] py-[0.5vw] rounded-full w-fit'>
            {tourData?.rate}
             <Image
              src={imgStar}
              width={100}
            ...
            />
          </div> : <Skeleton variant="circular" width={50} height={50} />}
        </div>
      </div>
    </Link>
  )
